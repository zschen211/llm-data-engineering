...
def answer(query: str, evidence: list[dict], backend: str = "fallback", model_name: str = "Qwen/Qwen2.5-VL-7B-Instruct") -> dict:
    if backend == "fallback":
    ...
    try:
        ...
        raise RuntimeError("vlm generation requires the optional 'gpu' extra (torch + transformers)") from exc
