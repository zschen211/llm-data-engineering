...
def get_parser():
    ...
    p_render = subparsers.add_parser("render-pdf", help="Render PDF pages into page image assets")
    ...
    p_index = subparsers.add_parser("build-index", help="Build page assets + index from a PDF")
    ...
    p_ask = subparsers.add_parser("ask", help="Retrieve evidence pages and answer a question")
    ...
    p_eval = subparsers.add_parser("evaluate", help="Evaluate retrieval over an eval set")
...
def cmd_index(args):
    ...
    index_mod.build_index(assets_dir / "page_units.jsonl", args.out, backend=args.backend)
