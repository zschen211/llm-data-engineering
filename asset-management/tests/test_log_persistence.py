...
from asset_management.log import _NeverDropQueueHandler, get_logger, setup_logging
...
def test_env_log_level(monkeypatch, tmp_path):
    monkeypatch.setenv("ASSET_LOG_LEVEL", "WARNING")
    buf = io.StringIO()
...
def test_reset_drains_previous_listener(tmp_path, tmp_path_factory):
    ...
    assert _wait_for((other_dir / "asset-management.log").exists)

    setup_logging(level=logging.INFO, stream=io.StringIO())  # file persistence off
    get_logger("test.persist").warning("after reset")
    assert _wait_for(
        lambda: "before reset" in (other_dir / "asset-management.log").read_text()
    )
    time.sleep(0.3)
    assert "after reset" not in (other_dir / "asset-management.log").read_text()


def test_create_app_configures_logging(tmp_path, monkeypatch):
    log_dir = tmp_path / "app-logs"
    monkeypatch.setenv("ASSET_LOG_DIR", str(log_dir))
    store = AssetStore(
        tmp_path / "assets.db",
    ...
    path = log_dir / "asset-management.log"
...
def test_rotation_compresses_backups(tmp_path, monkeypatch):
    monkeypatch.setenv("ASSET_LOG_MAX_BYTES", "300")
    monkeypatch.setenv("ASSET_LOG_BACKUPS", "3")
    setup_logging(level=logging.INFO, stream=io.StringIO(), log_dir=tmp_path)
    ...
    def backups():
        return sorted(tmp_path.glob("asset-management.log.*.gz"))
