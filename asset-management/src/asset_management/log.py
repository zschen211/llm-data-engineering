"""Unified logging for asset-management.

Every log line uses one format that identifies the emitting file, line and
function:

    2026-08-10 10:00:00 | INFO    | [asset-management] | store.py:142:sync_source | message
...
that a background thread drains into ``asset-management.log`` inside the log
...
_FORMAT = "%(asctime)s | %(levelname)-7s | [asset-management] | %(filename)s:%(lineno)d:%(funcName)s | %(message)s"
...
def setup_logging(
    ...
    """Configure the asset-management logger namespace once.
