...
asset-management 主包当前只有**数据资产层**（`assets/` 子包）：统一资产管理（数据源 / 下载管线 / 内容寻址存储 / 标签 / 版本 / 快照 / 管理 API）。包根仅保留资产层依赖的公共底座：`schema.py`（样本契约与 JSONL 工具）和 `log.py`（统一日志）。
...
| `log.py` | 日志基建 | 统一格式 `时间 \| 级别 \| [asset-management] \| 文件:行:函数 \| 消息` |
