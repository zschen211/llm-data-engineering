"""Observability for the asset-management service: metrics, sampling, events.
...
- Prometheus metrics (``prometheus-client``): process gauges (psutil), Ray
  ...
  (docker-compose provides one). Metric names carry the ``asset_`` prefix so
  a scrape of this endpoint never collides with the Ray metrics-agent scrape
  (fixed port ``ASSET_RAY_METRICS_PORT``, default 8080).
...
- A daemon sampler thread refreshes the gauges every ``$ASSET_OBS_INTERVAL``
...
_DIR_ENV = "ASSET_OBS_DIR"
_INTERVAL_ENV = "ASSET_OBS_INTERVAL"
_MAX_BYTES_ENV = "ASSET_OBS_MAX_BYTES"
_BACKUPS_ENV = "ASSET_OBS_BACKUPS"
...
_event_logger = logging.getLogger("asset_management.obs")
...
class EventSink:
    ...
    def write(self, name: str, level: str, fields: dict) -> None:
        ...
        log = logging.LogRecord(
            "asset_management.obs", logging.INFO, __file__, 0, line, None, None
        )
...
class Observability:
    ...
    def _build_metrics(self) -> None:
        ...
        gauges = {
            "asset_process_cpu_percent": "CPU usage of the uvicorn process, percent of one core",
            "asset_process_rss_bytes": "Resident set size of the uvicorn process",
            "asset_process_vms_bytes": "Virtual memory size of the uvicorn process",
            "asset_process_threads": "Thread count of the uvicorn process",
            "asset_process_open_fds": "Open file descriptors of the uvicorn process",
            "asset_process_children": "Child processes of the uvicorn process (raylet, workers)",
            "asset_ray_total_cpus": "CPU resources of the Ray cluster",
            "asset_ray_available_cpus": "CPU resources currently free in the Ray cluster",
            "asset_ray_alive_nodes": "Alive nodes in the Ray cluster",
            "asset_ray_running_tasks": "Tasks currently RUNNING in the Ray cluster",
            "asset_ray_alive_actors": "Actors currently ALIVE in the Ray cluster",
            "asset_raylet_cpu_percent": "CPU usage of the raylet process",
            "asset_raylet_rss_bytes": "Resident set size of the raylet process",
            "asset_ray_workers_total": "Number of live Ray worker processes",
            "asset_ray_workers_cpu_percent": "Aggregated CPU usage of Ray worker processes",
            "asset_ray_workers_rss_bytes": "Aggregated RSS of Ray worker processes",
            "asset_ray_metrics_up": "1 when the Ray metrics agent /metrics endpoint responds",
            "asset_ray_session_logs_bytes": "Total size of the Ray session log directory",
        }
        self._gauges = {
        ...
        self._stage_duration = Histogram(
            "asset_sync_stage_duration_seconds",
        ...
        self._item_duration = Histogram(
            "asset_sync_item_duration_seconds",
    ...
    def metrics_app(self):
        """ASGI app serving the Prometheus text exposition (tests)."""
        return make_asgi_app(self._registry)

    def metrics_text(self) -> bytes:
        """Prometheus text exposition bytes (served at ``/metrics``)."""
    ...
    def _sample_process(self) -> None:
        try:
            proc = psutil.Process()
            self._gauges["asset_process_cpu_percent"].set(proc.cpu_percent(None))
            memory = proc.memory_info()
            self._gauges["asset_process_rss_bytes"].set(memory.rss)
            self._gauges["asset_process_vms_bytes"].set(memory.vms)
            self._gauges["asset_process_threads"].set(proc.num_threads())
            self._gauges["asset_process_open_fds"].set(len(proc.open_files()))
            self._gauges["asset_process_children"].set(
    ...
    def _sample_ray(self) -> None:
        ...
        for name in (
            ...
            "asset_ray_available_cpus",
            "asset_ray_alive_nodes",
            "asset_ray_running_tasks",
            "asset_ray_alive_actors",
        ):
            self._gauges[name].set(status[name[len(prefix) :]])
        self._gauges["asset_ray_metrics_up"].set(
            _metrics_reachable(status.get("metrics_port", 0))
        )
        self._gauges["asset_ray_session_logs_bytes"].set(
            _dir_bytes(status.get("logs_dir", ""))
        )
        raylet, workers = self._ray_process_stats()
        self._gauges["asset_raylet_cpu_percent"].set(sum(_cpu(p) for p in raylet))
        self._gauges["asset_raylet_rss_bytes"].set(sum(_rss(p) for p in raylet))
        self._gauges["asset_ray_workers_total"].set(len(workers))
        self._gauges["asset_ray_workers_cpu_percent"].set(sum(_cpu(p) for p in workers))
        self._gauges["asset_ray_workers_rss_bytes"].set(sum(_rss(p) for p in workers))
...
def _resolve_log_dir(log_dir: object) -> Path | None:
    if log_dir is _AUTO:
        ...
        log_dir = env if env else os.environ.get("ASSET_LOG_DIR", "")
        if not log_dir:
            log_dir = str(Path(os.environ.get("ASSET_DATA_DIR", "data")) / "logs")
