...
that a background thread drains into ``llava-instruct.log`` inside the log
...
Rolling: size-based rotation (``$LLAVA_LOG_MAX_BYTES``, default 50 MB) with
``$LLAVA_LOG_BACKUPS`` compressed backups (default 5, gzip: ``.log.N.gz``,
newest is ``.log.1.gz``). Compressed on rotation, so the active file stays
plain for tail -f.

Log dir resolution: ``$LLAVA_LOG_DIR``, else ``$LLAVA_DATA_DIR/logs``, else
``data/logs`` relative to the working directory; ``setup_logging(log_dir=None)``
disables file persistence (used by tests). ``LLAVA_LOG_LEVEL`` overrides the
default level (INFO).
"""
...
def _resolve_log_dir(log_dir: object) -> Path | None:
    if log_dir is _AUTO:
        env = os.environ.get("LLAVA_LOG_DIR")
        log_dir = (
            Path(env)
            if env
            else Path(os.environ.get("LLAVA_DATA_DIR", "data")) / "logs"
        )
...
def _file_handler_for(log_dir: Path) -> logging.Handler:
    ...
    return _GzipRotatingFileHandler(
        log_dir / "llava-instruct.log",
        max_bytes=_env_int("LLAVA_LOG_MAX_BYTES", _MAX_BYTES),
        backup_count=_env_int("LLAVA_LOG_BACKUPS", _BACKUP_COUNT),
    )
...
def setup_logging(
    ...
    ``level``: logging level; defaults to ``$LLAVA_LOG_LEVEL`` or INFO.
    ``stream``: console sink (default stderr); tests pass a StringIO.
    ``log_dir``: log directory; defaults to ``$LLAVA_LOG_DIR`` else
    ``$LLAVA_DATA_DIR/logs`` else ``data/logs``; pass None to disable file
    persistence.
    """
    global _listener, _file_handler
    logger = logging.getLogger("llava_instruct")
    logger.setLevel(level or os.environ.get("LLAVA_LOG_LEVEL", "INFO"))
