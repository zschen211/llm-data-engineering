...
def _add_asset_parser(subparsers) -> None:
    ...
    s_add = source_sub.add_parser("add", help="Add a data source (huggingface only)")
    s_add.add_argument("--name", required=True)
    s_add.add_argument("--kind", required=True, choices=["huggingface"])
    s_add.add_argument("--url", default="")
    s_add.add_argument("--license", default="")
    s_add.add_argument("--description", default="")
    s_add.add_argument("--params", default=None, help="JSON: repo_id 必填；process=file|parquet, subfolder, allow_patterns, workers, image_column 等")
