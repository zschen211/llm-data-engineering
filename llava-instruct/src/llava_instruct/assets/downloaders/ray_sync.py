    ...
    def build(self) -> StorageBackend:
        ...
        if self.kind == "s3":
            return S3StorageBackend(
                self.endpoint or None, self.access_key, self.secret_key,
...
def _sync_file_task(cfg: SyncConfig, remote: RemoteRef) -> FileOutcome:
    ...
    try:
        ...
        processor = get_processor(cfg.source.params.get("process", "file"), cfg.source.params)
        persister = PersistStage(backend, db)
        work_dir = Path(cfg.tmp_dir) / remote.id
        work_dir.mkdir(parents=True, exist_ok=True)
        outcome = FileOutcome(remote=remote.name)

        def event(stage: str, remote: str = "", message: str = "",
                  level: str = "info", fraction: float | None = None) -> None:
            db.append_sync_event(cfg.run_id, stage, remote, level, message)
            db.update_sync_run(cfg.run_id, current_stage=stage, current_file=remote)

        try:
            _wait_until_resumed(db, cfg.run_id)
            event("download", remote.name, f"开始下载 → 暂存区 {work_dir / remote.name}")
            ...
            event("persist", remote.name, f"持久化完成：新增 {new}，跳过 {skipped}"
...
def run_ray_sync(cfg: SyncConfig, remotes: list[RemoteRef],
                 paused: Callable[[], bool] | None = None) -> list[FileOutcome]:
    ...
    if not ray.is_initialized():
        ...
        ray.init(num_cpus=max(1, cfg.workers), ignore_reinit_error=True,
    ...
    while pending:
        ...
        for ref in ready:
            ...
            try:
                ...
                outcome = FileOutcome(remote=remote.name, failed=1,
                                      errors=[f"{remote.name}: {exc}"])
