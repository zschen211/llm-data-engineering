...
统一资产管理：数据源（source）CRUD、HF 下载管线（download → process → persist，Ray 并行）、内容寻址存储（本地 / RustFS）、标签、版本、快照、Web 管理界面。元数据权威存 SQLite，blob 存可插拔 StorageBackend。
...
├── static/              # 静态前端资源（由 routes/info.py 读取返回）
│   └── webui.html       # Web 界面单文件前端（内联 css/js）
...
│   ├── info.py          # 应用级端点：webui 首页 / /api/info / 数据库备份
...
graph TD
    ...
    routes --> webui["static/webui.html"]
