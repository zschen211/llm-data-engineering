...
三个子项目对应《大模型数据工程》第十四篇的三个实战项目：

| 子项目 | 对应章节 | 说明 | 核心链路 |
| --- | --- | --- | --- |
| [llava-instruct](llava-instruct/) | 项目三 | LLaVA 多模态指令数据工厂 | 资产池 → 指令合成 → 区域对齐 → 多图交错 → 质检 → 训练封装 |
| [mm-rag](mm-rag/) | 项目五 | 多模态 RAG 企业财报助手 | PDF → 页面渲染 → 视觉索引 → Top-K 召回 → 多图推理 → 评测 |
| [video-generation](video-generation/) | 项目十四 | 视频生成数据流水线（T2V） | 源加载 → 镜头切分 → 运动/美学过滤 → 多帧 caption → 镜头语言标注 |
