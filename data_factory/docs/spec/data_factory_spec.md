...
data-factory 是 asset 资产层之上的**数据生产与评测闭环**，包含两个子系统：
...
└──────────────┬────────────────────────────────────────┬───────────┘
               ...
               ▼                                          ▼
   asset 资产层（blob/快照/标签/物化）        RustFS + SQLite
```
...
| 资产层（asset） | 素材 blob、快照、标签、物化 | 输入数据的唯一权威（只读消费） |
...
| 编号 | 决策 | 理由 |
| --- | --- | --- |
| D1 | 新建独立子项目 `data-factory`，依赖 asset（path 依赖） | 仓库独立子项目约定；资产层 API 是唯一稳定入口 |
...
```toml
# data_factory/pyproject.toml（设计草案）
[project]
name = "data-factory"
requires-python = ">=3.11"
dependencies = [
    "asset = {path = \"../asset\"}",  # 资产层 API（同仓 path 依赖）
...
- LLM 能力（local 推理、`qc_llm`、`llm_judge`）走 `gpu` extra——沿用 mm_rag/video_generation 的已批准例外模式，不扩展至其他代码；
...
- 子项目间依赖漂移：asset 以固定版本/path 锁定，资产层 API 变化走版本协商。
