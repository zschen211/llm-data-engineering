...
def sanitize_and_coerce_to_vocab(
    raw: dict, vocab: dict[str, list[str]] | None = None
) -> dict:
...
def flow_statistics(
    segment_path: str, proxy_wh: tuple[int, int] = (480, 270), max_pairs: int = 30
) -> dict:
    ...
    try:
        while len(mags) < max_pairs:
            ...
            if prev is not None:
                ...
                def small(img):
                    return cv2.resize(img, proxy_wh)
...
def tag_shot_language(
    shot_id: str, segment_path: str, frame_paths: list[str], vlm_fn=None
) -> dict:
