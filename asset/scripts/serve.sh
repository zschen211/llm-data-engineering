...
# local content-addressed directory (data/blobs/). Ray: attach the shared
# cluster via $RAY_ADDRESS (infra/scripts/ray-start.sh); when unset an
# embedded local cluster is started as a dev fallback (loud warning).
# A `.env` file in the project root (template: ../infra/.env.example) is
# loaded first and wins over the built-in defaults, but is overridden by
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --host) host="$2"; shift 2 ;;
    --port) port="$2"; shift 2 ;;
    --data-dir) export ASSET_DATA_DIR="$2"; shift 2 ;;
    --storage)
      case "$2" in
...
if [[ "$storage" == "rustfs" ]]; then
  ...
  export RUSTFS_BUCKET="${RUSTFS_BUCKET:-asset-assets}"
  export ASSET_STORAGE_BACKEND="${ASSET_STORAGE_BACKEND:-rustfs}"
...
echo "serving asset manager on http://${host}:${port} (data: ${ASSET_DATA_DIR:-data})" >&2
exec uv run uvicorn asset_management.assets.routes:default_app --factory --host "$host" --port "$port"
