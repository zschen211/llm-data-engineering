...
access the asset layer exclusively through this module::
    ...
    from asset_management.assets.api import open_store, AssetStore
...
DEFAULT_DATA_DIR = Path(os.environ.get("ASSET_DATA_DIR", "data"))


def _resolve_backend(data_dir: Path) -> StorageBackend:
    """Resolve the storage backend from the environment.

    ``ASSET_STORAGE_BACKEND``: ``rustfs`` (requires endpoint + credentials,
    ...
    """
    switch = os.environ.get("ASSET_STORAGE_BACKEND", "auto").lower()
    endpoint = os.environ.get("RUSTFS_ENDPOINT")
    if switch not in ("auto", "local", "rustfs"):
        raise ValueError(
            f"unknown ASSET_STORAGE_BACKEND {switch!r} (auto|local|rustfs)"
        )
    if switch == "rustfs" and not endpoint:
        raise ValueError("ASSET_STORAGE_BACKEND=rustfs requires RUSTFS_ENDPOINT")
    ...
    if not use_rustfs:
        logger.warning(
            ...
            "ASSET_STORAGE_BACKEND=rustfs to require RustFS",
            data_dir / "blobs",
        )
    ...
    return S3StorageBackend(
        ...
        os.environ.get("RUSTFS_BUCKET", "asset-assets"),
    )
...
def open_store(
    ...
    ``ASSET_STORAGE_BACKEND`` switch selects the target (see
    ``_resolve_backend``).
    """
