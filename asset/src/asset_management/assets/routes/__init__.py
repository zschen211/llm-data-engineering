...
``make_router(store) -> APIRouter``. ``create_app`` mounts them all; index
pointer, system info and backup live in ``routes/info.py``. The management
UI itself is the standalone ``frontend/`` SPA, consuming these ``/api/*``
endpoints over HTTP only.
"""
...
def create_app(store: AssetStore) -> FastAPI:
    setup_logging()
    app = FastAPI(title="asset asset manager", version="0.1.0", lifespan=_lifespan)
    app.add_middleware(MetricsMiddleware)

    # Plain route (not a mount): the exposition is served at exactly
    # /metrics with a 200 — a mounted ASGI sub-app would 307-redirect to
    # /metrics/ (redirect_slashes), breaking plain curl and -f checks.
    @app.get("/metrics")
    def metrics():
        return Response(
            content=observability.metrics_text(),
