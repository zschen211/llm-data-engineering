...
asset 数据工厂的日常操作指南：环境准备、CLI 命令、Web 管理界面、统一 API 与存储后端运维。
...
uv run asset prepare-assets ./images --out assets.jsonl --labels labels.json

# 2) 指令合成：资产 + 证据文件 -> LLaVA 对话样本
uv run asset generate assets.jsonl \
  --captions captions.jsonl --ocr ocr.jsonl --bbox bbox.jsonl --pairs pairs.jsonl \
  --out samples.jsonl

# 3) 质检：结构 / 语义 / bbox 越界检查
uv run asset qa samples.jsonl --image-root ./images --report qa_report.md

# 4) bbox 反向渲染可视化核验
uv run asset render samples_qa.jsonl --image-root ./images --out-dir render

# 5) train/val/smoke 切分 + manifest
uv run asset split samples_qa.jsonl --out-dir deliver
```

## 3. 资产层管理（`asset` 命令族）

```bash
uv run asset asset init                         # 查看当前配置（后端/数据目录/bucket）
uv run asset asset source add --name coco --kind huggingface \
  --params '{"repo_id": "org/ds", "subfolder": "images", "allow_patterns": ["*.png"]}'
uv run asset asset source list
uv run asset asset source rm <source_id>

# HF parquet 数据集（如 COCO-Caption）：process=parquet 逐行解码为图片资产
export HF_ENDPOINT=https://hf-mirror.com        # 国内访问 HF 需走镜像
uv run asset asset source add --name coco-caption --kind huggingface \
  --params '{"repo_id": "lmms-lab-encoder/COCO-Caption", "subfolder": "data", "allow_patterns": ["*.parquet"], "process": "parquet"}'

uv run asset asset sync <source_id>             # 下载管线：Ray 逐文件任务（并行+崩溃自动重试+转换+入库，幂等可重试）
uv run asset asset import ./images --out assets.jsonl   # 本地导入（等效 prepare-assets）

uv run asset asset ls --tag task=chart --type chart_image --status ready
uv run asset asset tag add <asset_id> chart --group task
uv run asset asset tag rm <asset_id> chart
uv run asset asset version ls <asset_id>        # 版本历史
uv run asset asset version rollback <asset_id> 1
uv run asset asset version snapshot --name v1   # 集合级快照（固定资产清单，可复现）
uv run asset asset version snapshot-list
uv run asset asset materialize ./pool --tag task=chart   # 物化到本地供下游
```

## 4. Web 管理界面

```bash
uv run asset asset serve --port 8000            # 浏览器访问 http://localhost:8000
```
