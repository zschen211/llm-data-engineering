...
def test_compute_motion_magnitude_moving(tmp_path):
    ...
    stats = motion.compute_motion_magnitude(
        str(path), proxy_wh=(32, 32), stride=1, max_pairs=60
    )
...
def test_summarize_camera_motion_static():
    assert (
        tag.summarize_camera_motion({"mean_magnitude": 0.0, "std_magnitude": 0.0})
        == "static"
    )
