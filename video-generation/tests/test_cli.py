...
def test_cli_tag_shot_language(tmp_path, monkeypatch):
    ...
    writer = cv2.VideoWriter(
        str(shot_path), cv2.VideoWriter_fourcc(*"MJPG"), 10.0, (64, 64)
    )
    for _ in range(5):
...
def test_cli_build_manifest(tmp_path):
    for name in (
        ...
        "stage2_scenes.jsonl",
        "stage3_motion.jsonl",
        "stage4_aesthetic.jsonl",
        "stage5_captions.jsonl",
        "stage6_shot_language.jsonl",
    ):
        (tmp_path / name).write_text("")
    (tmp_path / "source_videos.jsonl").write_text(
        '{"video_id": 1, "license": "pexels"}\n'
    )
    (tmp_path / "stage2_scenes.jsonl").write_text(
        '{"shot_id": "s1", "video_id": 1, "start_ts": 0.0, "end_ts": 1.0, "segment_path": "shots/x.mp4", "status": "ok"}\n'
    )
    (tmp_path / "stage5_captions.jsonl").write_text(
        '{"shot_id": "s1", "caption_en": "A sunny beach."}\n'
    )
    (tmp_path / "stage6_shot_language.jsonl").write_text(
        '{"shot_id": "s1", "camera_motion": "static"}\n'
    )
    ...
    assert (
        main(
            [
                "build-manifest",
                "--sources",
                str(tmp_path / "source_videos.jsonl"),
                "--scenes",
                str(tmp_path / "stage2_scenes.jsonl"),
                "--motion",
                str(tmp_path / "stage3_motion.jsonl"),
                "--aesthetic",
                str(tmp_path / "stage4_aesthetic.jsonl"),
                "--captions",
                str(tmp_path / "stage5_captions.jsonl"),
                "--shot-language",
                str(tmp_path / "stage6_shot_language.jsonl"),
                "--out",
                str(out),
            ]
        )
